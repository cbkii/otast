...
# prepares/uploads optional physical proof, then reruns that same workflow.
...
RECONCILE_SCRIPT="$SCRIPT_DIR/reconcile-release-state.py"
...
[[ -f $RECONCILE_SCRIPT && ! -L $RECONCILE_SCRIPT ]] || {
    fatal "release state reconciler is missing or unsafe: $RECONCILE_SCRIPT"
    exit 1
}
...
run_bounded() {
    ...
    rc=$?
    case $rc in
        124|137) warn "$description timed out after ${seconds}s" ;;
    esac
    return "$rc"
}
...
refresh_local_main_once() {
    ...
    if ! run_bounded 'fetch origin/main' "$GIT_TIMEOUT_SECONDS" \
        "$REAL_GIT" -C "$REPO_ROOT" fetch --quiet origin main; then
    ...
    if ! run_bounded 'fast-forward local main' 20 \
        "$REAL_GIT" -C "$REPO_ROOT" merge --ff-only --quiet origin/main; then
    ...
    if [[ -n $before && -n $after && $before != "$after" && $WRAPPER_REEXECED != 1 ]]; then
        ...
        exec env OTAST_RELEASE_WRAPPER_REEXECED=1 \
            bash "$REPO_ROOT/scripts/release-device.sh" "${ORIGINAL_ARGS[@]}"
    fi
}
...
release_state() {
    run_bounded "read GitHub release $VERSION" "$NETWORK_TIMEOUT_SECONDS" \
        "$REAL_GH" release view "$VERSION" -R "$REPO_SLUG" \
        --json isDraft,assets,targetCommitish 2>/dev/null
}
...
if ((release_state_rc != 0)); then
    ...
    case $absence_rc in
        ...
        1)
            ...
            release_state_rc=$?
            ((release_state_rc == 0)) || {
                fatal "GitHub reports $VERSION exists but its release state could not be read reliably"
                exit 1
            }
            ;;
        *)
            fatal "cannot determine whether GitHub release $VERSION exists; private release state was not changed"
            exit 1
            ;;
    esac
fi
...
reconcile_private_state() {
    ...
    if [[ -L $state_base ]]; then
        fatal "private release state base is a symlink: $state_base"
        return 1
    fi
    mkdir -p -- "$state_base" || {
        fatal "cannot create private release state base: $state_base"
        return 1
    }
    chmod 0700 -- "$state_base" 2>/dev/null || {
        fatal "cannot secure private release state base: $state_base"
        return 1
    }
    ...
    if [[ -n $existing ]]; then
        release_file=$WORK/release-state.json
        printf '%s\n' "$existing" >"$release_file" || return 1
        chmod 0600 -- "$release_file" 2>/dev/null || true
        reconcile_args+=(--release-json "$release_file")
    elif ((release_absent)); then
        reconcile_args+=(--release-absent)
    else
        fatal 'release-state reconciliation has no authoritative remote state'
        return 1
    fi

    result=$(python3 "$RECONCILE_SCRIPT" "${reconcile_args[@]}")
    rc=$?
    if ((rc != 0)); then
        [[ -z $result ]] || printf '%s\n' "$result" >&2
        fatal "private release state reconciliation failed (status $rc); no live device state was changed"
        return "$rc"
    fi
    action=$(python3 -c 'import json,sys; print(json.load(sys.stdin).get("action") or "")' <<<"$result") || return 1
    if [[ $action == ARCHIVED ]]; then
        reason=$(python3 -c 'import json,sys; print(json.load(sys.stdin).get("reason") or "")' <<<"$result") || reason=
        archive=$(python3 -c 'import json,sys; print(json.load(sys.stdin).get("archive") or "")' <<<"$result") || archive=
        warn "Reconciled orphaned local qualification state: ${reason:-candidate binding mismatch}."
        info "Archived previous private qualification state intact: $archive"
        info 'Fresh qualification will start from START; any live OTAST managed state will be Verify/Restore checked by the lifecycle.'
    fi
    return 0
}

reconcile_private_state || exit $?

# The retained lifecycle speaks its original operation=draft/publish API.
...
if [[ ${1:-} == release && ${2:-} == delete ]]; then
    ...
    if ! run_real "${view[@]}" >/dev/null 2>&1; then exit 0; fi
    exit "$rc"
fi
...
if [[ ${1:-} == workflow && ${2:-} == run ]]; then
    ...
    while ((index < ${#args[@]})); do
        ...
        if [[ $value == -f && $((index + 1)) -lt ${#args[@]} ]]; then
            pair=${args[index+1]}
            case $pair in
                operation=draft) operation=draft; index=$((index + 2)); continue ;;
                operation=publish) operation=publish; index=$((index + 2)); continue ;;
            esac
        fi
        out+=("$value")
        index=$((index + 1))
...
"$timeout_cmd" --kill-after=5s "${timeout_seconds}s" "$real" "$@"
rc=$?
case $rc in 124|137) printf '[OTAST][WARN] git command timed out after %ss\n' "$timeout_seconds" >&2 ;; esac
exit "$rc"
GIT_SHIM
...
if [[ $AUTO_PUBLISH != 1 ]]; then exit 0; fi
