...
otast_plan_ta_utl() {
  ...
  for id in TA_utl .TA_utl; do
    for dir in $(otast_effective_module_dirs "$id"); do
      ...
      webui_found=0
      if [ -d "$dir/webui/assets" ] && [ ! -L "$dir/webui/assets" ]; then
      ...
      elif [ -e "$dir/webui/assets" ] || [ -L "$dir/webui/assets" ]; then
        otast_stop "TA UTL WebUI assets path is unsafe: $dir/webui/assets"
        return 1
      fi

      if [ "$webui_found" -eq 1 ]; then
